...
import { getHackerRank } from "@/utils/mdx";

const POSTS_PER_PAGE = 5;

const HackerRanks = () => {
  const { data: hackerranks = [], isLoading } = useQuery({
    queryKey: ['hackerranks'],
  ...
  const filtered = useMemo(() => {
    return hackerranks.filter((sheet) => {
      ...
      <main className="max-w-2xl mx-auto px-6 py-12">
        <header className="mb-10">
          ...
          <div className="mt-6">
            <input
              type="text"
              placeholder="Search hackerranks..."
              value={searchTerm}
              onChange={(e) => {
        ...
        {isLoading ? (
          <div className="text-center text-gray-500">Loading hackerranks...</div>
        ) : (
          <>

...
import { getCodewarsBySlug } from "@/utils/mdx";

const Codewar = () => {
  const { slug } = useParams();

  const { data: codewar, isLoading, error } = useQuery({
    queryKey: ["codewars", slug],
    queryFn: () => getCodewarsBySlug(slug!),
    enabled: !!slug,
  });
    ...
    return (
      <div className="min-h-screen bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100 transition-colors">
        ...
        <main className="max-w-2xl mx-auto px-6 py-12">
          <div className="text-center text-gray-500 dark:text-gray-400">Loading codewar...</div>
        </main>
        <Footer />
      </div>
    );
        ...
        <article>
          <header className="mb-10">
            ...
            <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 space-x-4">
              <time dateTime={codewar.date}>
                {new Date(codewar.date).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })}
              ...
              {codewar.readTime && (
                <>
                  <span>•</span>
                  <span>{codewar.readTime}</span>
                </>
              )}
            </div>

            {codewar.tags && codewar.tags.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {codewar.tags.map((tag) => (
                  <span
                    key={tag}
          ...
          <MDXContent code={codewar.content} />
        </article>
      </main>
